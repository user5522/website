import { g as getContext, c as create_ssr_component, f as subscribe, e as escape } from "../../chunks/index.js";
const getStores = () => {
...
const Error$1 = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_page();
  return `${$$result.head += `<!-- HEAD_svelte-o1aspw_START -->${$$result.title = `<title>${escape($page.error.message)}</title>`, ""}<!-- HEAD_svelte-o1aspw_END -->`, ""}
...
<div class="${"flex flex-col items-center justify-center gap-5 p-5"}"><div class="${"flex flex-col items-center justify-center gap-1"}"><div class="${"text-4xl font-semibold"}">${escape($page.status)}: ${escape($page.error.message)}</div>
	...
	<div><div class="${"text-2xl"}">This looks like an error.. &amp; that&#39;s unfortunate.</div>
		<div class="${"flex flex-col gap-5"}"><div class="${"flex justify-center text-2xl"}">Maybe try one of these solutions:</div>
			...
			<div class="${"flex justify-center text-xl"}">If none of these solutions work:</div></div></div>
